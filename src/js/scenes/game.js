...
export class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "rightWall",
      "../Resources/Art/Scenery/Platforms/BasicWalls/rightWall.png"
    );
    this.load.image(
      "leftWall",
      "../Resources/Art/Scenery/Platforms/BasicWalls/leftWall.png"
    );
    this.load.image(
      "ground",
      "../Resources/Art/Scenery/Platforms/BasicWalls/ground.png"
    );
    this.load.image(
      "ceiling",
      "../Resources/Art/Scenery/Platforms/BasicWalls/ceiling.png"
    );
    this.load.image(
      "central",
      "../Resources/Art/Scenery/Platforms/BasicWalls/central.png"
    );
    this.load.image(
      "grass",
      "../Resources/Art/Scenery/Platforms/BasicWalls/grass.png"
    );
    ...
    // basic platforms
    this.load.image(
      "base_bigcandy_platform",
      "../Resources/Art/Scenery/Platforms/Base/base_bigcandy_platform.png"
    );
    this.load.image(
      "base_hw_platform",
      "../Resources/Art/Scenery/Platforms/Base/base_hw_platform.png"
    );
    this.load.image(
      "base_ribbon_platform",
      "../Resources/Art/Scenery/Platforms/Base/base_ribbon_platform.png"
    );
    this.load.image(
  ...
  create() {
    ...
    if (this.player1team == "halloween") {
      // player 1 is edward cullon and player 2 maria karei
      spriteP1 = "edward";
      spriteP2 = "maria";
    } // player 2 is edward cullon and player 1 maria karei
    else {
      spriteP1 = "maria";
      spriteP2 = "edward";
    }
    ...
    player1 = this.physics.add.existing(
      new Player(this, 350, 3, 200, 100, spriteP1, 1)
    ); //scene, speed, lives, x, y, sprite, index

    player2 = this.physics.add.existing(
      new Player(this, 300, 3, 500, 100, spriteP2, 2)
    );
    ...
    this.physics.add.collider(
      ...
      null,
      this
    ); // collider between player and bullets
    this.physics.add.collider(
      this.platforms,
      this.bullets,
      bulletOnWall,
      null,
      this
    ); // collider between bullets and platforms
    ...
    this.physics.add.collider(
      ...
      null,
      this
    ); // collider between player and weapons
    ...
    //Shooting
    this.numpad_0_key = this.input.keyboard.addKey(
      Phaser.Input.Keyboard.KeyCodes.NUMPAD_ZERO
    );
    this.numpad_7_key = this.input.keyboard.addKey(
      Phaser.Input.Keyboard.KeyCodes.NUMPAD_SEVEN
    );
    this.numpad_9_key = this.input.keyboard.addKey(
  ...
  update() {
    ...
    this.checkWinners(); // Checks the game status every frame to see if someone won
  }

  changeStage() {
    if (player1.getLives() == player2.getLives())
      // if the players have the same lives
      this.stage.setFrame(1);
    // mixed stage
    else if (player1.getLives() > player2.getLives()) {
      // if player1 has more lives
      if (this.player1team == "halloween") {
        // if player1 is on the halloween team
        this.stage.setFrame(0); // halloween stage
      } else this.stage.setFrame(2); // christams stage
    } else if (player1.getLives() < player2.getLives()) {
      // if player2 has more lives
      if (this.player2team == "halloween")
        // if player2 is on the halloween team
...
function outOfTime() {
  // i was playing with this for the 5 minute timeout
...
function bulletOnPlayer(player, bullet) {
  ...
  if (player.canBeDamaged) {
    ...
    if (player.takeDamage()) {
      // If the player takes the damage and doesn't die...
      player.alpha = 0.5;
...
function bombCollision(first, bomb) {
  ...
  var explosion = this.explosions
    .create(bomb.x, bomb.y, "explosion")
    .setScale(1.3);
